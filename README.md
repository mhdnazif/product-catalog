...
Show the catalog, scrolling pagination, search, empty results, a product detail screen, back navigation, and pull-to-refresh. Demonstrate a recoverable error and image fallback if practical. Explain the data/UI separation, server-side search decision, cancellation checks, and the test suite. The refresh behaviour that keeps previous results visible is a useful UX detail to highlight.

## Commit history

Development checkpoints are preserved in Git. Review them on GitHub or run:

```bash
git log --oneline --reverse
```

The source ZIP is a snapshot; the repository is the source of the progressive commit history.
